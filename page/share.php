...
namespace xepan\hr {
	...
	class page_share extends \Page {
		...
		if($crud->isEditing()){
			// $form = $this->add('Form');
			$form = $crud->form;
			// $type_field = $form->addField('DropDown','shared_type')->setValueList(['Global'=>'Global','Department'=>'Department','Person'=>'Person']);
			// $form->addField('DropDown','employee')->setModel('xepan\hr\Employee');
			// $form->addField('name');
